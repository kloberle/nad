...
from .nad_transport import (NadTransport, SerialPortTransport, TelnetTransportWrapper, DEFAULT_TIMEOUT)
...
class NADReceiver:
    ...
    def zone2_source(self, operator: str, value: Optional[str]=None) -> Optional[Union[int, str]]:
        ...
        """
        if value is not None:
            source = self.exec_command('zone2', 'source', operator, str(value))
        else:
            source = self.exec_command('zone2', 'source', operator)
    ...
    def zone2_volume(self, operator: str, value: Optional[str] =None) -> Optional[float]:
        ...
        """
        if value is not None:
            volume = self.exec_command('zone2', 'volume', operator, str(value))
        else:
            volume = self.exec_command('zone2', 'volume', operator)
    ...
    def zone3_mute(self, operator: str, value: Optional[str] =None) -> Optional[str]:
        """Execute Zone3.Mute."""
        return self.exec_command('zone3', 'mute', operator, value)

    def zone3_power(self, operator: str, value: Optional[str] =None) -> Optional[str]:
        """Execute Zone3.Power."""
        return self.exec_command('zone3', 'power', operator, value)

    def zone3_source(self, operator: str, value: Optional[str]=None) -> Optional[Union[int, str]]:
        """
        Execute Zone3.Source.

        Returns int
        """
        if value is not None:
            source = self.exec_command('zone3', 'source', operator, str(value))
        else:
            source = self.exec_command('zone3', 'source', operator)
    ...
    def zone3_volume(self, operator: str, value: Optional[str] =None) -> Optional[float]:
        """
        Execute Zone3.Volume.

        Returns float
        """
        if value is not None:
            volume = self.exec_command('zone3', 'volume', operator, str(value))
        else:
            volume = self.exec_command('zone3', 'volume', operator)
